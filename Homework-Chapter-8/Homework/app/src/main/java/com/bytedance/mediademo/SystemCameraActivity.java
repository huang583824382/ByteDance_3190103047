...
public class SystemCameraActivity extends AppCompatActivity {
    ...
    private void takePhotoUsePathHasPermission() {
        ...
        intent.putExtra(MediaStore.EXTRA_OUTPUT, PathUtils.getUriForFile(getApplicationContext(),takeImagePath));
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_TAKE_PHOTO && resultCode == RESULT_OK) {
            ...
            int targetwid = imageView.getWidth();
            int targethei = imageView.getHeight();
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inJustDecodeBounds = true;

            BitmapFactory.decodeFile(takeImagePath, options);
            int photowid = options.outWidth;
            int photohei = options.outHeight;
            int scale = Math.min(photowid/targetwid, photohei/targethei);
