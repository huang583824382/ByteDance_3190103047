...
public class NoteActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                saved=true;
                getPreferences(MODE_PRIVATE).edit().putString(KEY_DRAFT, "").apply();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode==KeyEvent.KEYCODE_BACK){
            if(!saved&&!TextUtils.isEmpty(editText.getText().toString())) dialog = builder.show();
//            dialog.dismiss();
        }
    ...
    private boolean saveNote2Database(String content, Priority priority) {
        ...
        Log.d("=------", "saveNote2Database: "+rowId);
        return rowId != -1; //插入成功时rowId是插入的数据的ID
    }
